...
function smoothScrollTo(target) {
 ...
 function handleFormSubmit(event) {
     ...
     // 显示成功消息
     alert(currentLang === 'zh' ? '感谢您的留言，我们会尽快回复！' : 
           currentLang === 'en' ? 'Thank you for your message, we will reply soon!' : 
           '메시지를 보내주셔서 감사합니다. 곧 답변드리겠습니다!');
 ...
 function handleInquiryFormSubmit(event) {
     ...
     // 发送邮件到您的邮箱
     sendInquiryToBackend(data, submitBtn, originalText);
 }
 
 // 发送询价到后端
function sendInquiryToBackend(formData, submitBtn, originalText) {
    // 准备数据
    const inquiryData = {
        ...
        product_interest: formData.products,
        quantity: formData.quantity,
        target_market: formData.market || '未指定',
        message: formData.additional || '无'
    };
    ...
    emailjs.send('service_y2ur8xo', 'template_cygz6pq', {
        ...
        from_name: inquiryData.contact_person,
        ...
        products: inquiryData.product_interest,
        quantity: inquiryData.quantity,
        market: inquiryData.target_market,
        message: inquiryData.message,
        reply_to: inquiryData.email,
        // 添加更多可能的变量名
        name: inquiryData.contact_person,
        email: inquiryData.email,
        company_name: inquiryData.company_name,
        product_interest: inquiryData.product_interest,
        target_market: inquiryData.target_market,
        additional_info: inquiryData.message
    })
    ...
    .catch(function(error) {
        ...
        // 显示错误提示
        const errorMsg = currentLang === 'zh' ? '提交失败，请稍后重试或直接联系我们' : 
